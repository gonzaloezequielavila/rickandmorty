...
describe('Test de RUTAS', ()=>{
  ...
  describe("GET /rickandmorty/login", ()=>{
    it('Responde según las credenciales correctas', async ()=>{
      const {body} = await agent.get('/rickandmorty/login?email=example@gmail.com&password=pass123');
      expect(body).toEqual({
        access: true,
      })
    });
    ...
    it('Responde según las credenciales incorrectas', async ()=>{
      const {body} = await agent.get('/rickandmorty/login?email=exampl@gmail.com&password=pass12');
  ...
  describe("DELETE /rickandmorty/fav/:id", ()=>{
    it('Devolver el arreglo si el id no se encuentra', async ()=>{
    ...
    it('Elimina el personaje si el id el válido', async()=> {
